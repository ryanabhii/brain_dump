...
class DriveRemoteStore implements RemoteStore {
  ...
  // All tab keys the app knows about. Used by accessibleTabs() to query each
  // tab individually because the Drive API requires key+value in appProperties
  // queries — querying by key alone is unsupported (400 Invalid Value).
  static const _knownTabKeys = [
    'killzones',
    'spend',
    'capture',
    'household',
  ...
  Future<Map<String, SyncRole>> accessibleTabs() async {
    ...
    // each known tab key individually and merge the results.
    final out = <String, SyncRole>{};
    for (final tabKey in _knownTabKeys) {
      final res = await api.files.list(
        q: "appProperties has { key='$_propKey' and value='$tabKey' } and trashed=false",
